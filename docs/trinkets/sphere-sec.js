...
function sphere_sec_angbetween(d1, a1, d2, a2) {
...
function render_sphere_sec(ssrdiv, ang1, ang2, ang3, ang4) {
	...
	<tr><td>Circumference Distance (Radii)</td><td>${drad.toFixed(2)}</td>
	<tr><td>Chord Distance (Radii)</td><td>${crd.toFixed(2)}</td></tr>
	<tr><td>Ratio</td><td>${(drad/crd).toFixed(3)}</td></tr>
	<tr><td>Chord Distance (Earth, km)</td><td>${(crd*6371).toFixed(2)}km</td></tr>
	</table>
	`
}
