...
But what does it _represent_, really? Well, it represents a fraction of a rotation (the Earth's), which makes it an angle.

In many astronomical calculations, angles in coordinates are represented as a time to make certain conversions easier.
For example, the horizontal coordinate of [Algol](https://en.wikipedia.org/wiki/Algol) is 03:08:10.13 (Usually written 03h08m10.13s).

If you disregard the fact that this is a coordinate and treat it as a time of day,
it can be converted to [local mean sidereal time](https://en.wikipedia.org/wiki/Sidereal_time). Which contains
...
diameter in the sky… Good to know for telescope adjustment ("If I leave my telescope alone,
...
results that are interesting, but don't deserve much commentary.

![Places on Earth where it can be 100 degrees in the morning](/graph/places100deg.png)

# The Tangent of Time

Why radians? For trig functions, of course!
...
[^1]: This makes sense because the term "12-hour clock" is valid even though it describes a 24 hour day
